...
if(!$_SESSION['admin_userid']){
	...
	$sqlTot="select count(*) from indent";
	...
	$sql="select indent.*,user.username,status.name from indent,user,status where indent.user_id=user.id and indent.status_id=status.id group by indent.dingdanhao order by id desc limit {$page->offset},{$page->length} ";
	$smt=$pdo->prepare($sql);
	$smt->execute();
	$rows=$smt->fetchAll();
?>
	...
	<!-- <div class="container"> -->
		...
		<table class="table table-striped table-bordered table-hover">
			...
			foreach($rows as $row)
			{
				echo '<tr>';
				echo "<td>{$row['id']}</td>";
				echo "<td><a href='dingdanhao.php?dingdanhao={$row['dingdanhao']}' class='btn btn-info'>订单号</a></td>";
                echo "<td>{$row['username']}</td>";
                echo "<td>".date('Y-m-d H:i:s',$row['time'])."</td>";
                echo "<td>{$row['name']}</td>";
                echo "<td><a href='touch.php?touch_id={$row['touch_id']}' class='btn btn-success'>联系方式</a></td>";
                if($row['confirm']){
                    echo "<td><a href='touch.php?touch_id={$row['touch_id']}' class='btn btn-warning'>是</a></td>";
                }else{
                    echo "<td><a href='touch.php?touch_id={$row['touch_id']}' class='btn btn-warning'>否</a></td>";
                }
                
                echo "<td><a href='edit.php?dingdanhao={$row['dingdanhao']}&status_id={$row['status_id']}' class='btn btn-primary'>修改</a></td>";
				echo "<td><a href='delete.php?dingdanhao={$row['dingdanhao']}' class='delete btn btn-danger'>删除</a></td>";
				echo '</tr>';
			}
			?>
		</table>
