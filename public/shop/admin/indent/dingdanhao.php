...
if(!$_SESSION['admin_userid']){
	...
	$sql="select indent.dingdanhao,indent.price,indent.num,shop.name,shop.img from indent,shop where indent.shop_id=shop.id and indent.dingdanhao='{$dingdanhao}'  ";
	$smt=$pdo->prepare($sql);
	$smt->execute();
	$rows=$smt->fetchAll();
?>
...
<body>
	...
	<!-- <div class="container"> -->
		...
		<table class="table table-striped table-bordered table-hover">
			...
			{
				echo '<tr>';
                echo "<td>{$row['dingdanhao']}</td>";
				echo "<td>{$row['name']}</td>";
				echo "<td><img src='../public/upload/{$row['img']}' height='70px' width='70px'</td>";
                echo "<td>{$row['num']}</td>";
        ...
        <!-- <div>
            <?php echo "<span>".($row['num']*$row['price']+$row['num']*$row['price'])."</span>";?>
        </div> -->
