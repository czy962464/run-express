...
if(!$_SESSION['admin_userid']){
	...
	$sql="select shop.*,brand.name bname from shop,brand where shop.brand_id=brand.id order by id desc limit {$page->offset},{$page->length} ";
	$smt=$pdo->prepare($sql);
	$smt->execute();
	$rows=$smt->fetchAll();
?>
	...
	<!-- <div class="container"> -->
		...
		<table class="table table-striped table-bordered table-hover">
			...
			foreach($rows as $row)
			{
                ...
                echo "<td>{$row['name']}</td>";
                echo "<td><img src='../public/upload/{$row['img']}' height='60px' width='60px' </td>";
				...
				echo "<td><a href='edit.php?id={$row['id']}' class='btn btn-primary'>修改</a></td>";
				echo "<td><a href='delete.php?id={$row['id']}&img={$row['img']}' class='delete btn btn-danger'>删除</a></td>";
				echo '</tr>';
			}
			?>
		</table>
