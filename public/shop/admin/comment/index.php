...
if(!$_SESSION['admin_userid']){
	...
	$sqlTot="select count(*) from comment";
	...
	$sql="select comment.*,user.username,shop.name from comment,user,shop where comment.user_id=user.id and comment.shop_id=shop.id order by id desc limit {$page->offset},{$page->length} ";
	$smt=$pdo->prepare($sql);
	$smt->execute();
	$rows=$smt->fetchAll();
?>
