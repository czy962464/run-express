...
if(!$_SESSION['admin_userid']){
	...
	$sqlTot="select count(*) from status";
	...
	$sql="select * from status order by id asc limit {$page->offset},{$page->length} ";
	$smt=$pdo->prepare($sql);
	$smt->execute();
	$rows=$smt->fetchAll();
?>
	...
	<!-- <div class="container"> -->
		...
		<table class="table table-striped table-bordered table-hover">
			...
			foreach($rows as $row)
			{
                ...
                if($row['id']==1){
                    echo "<td><a href='javascript:' style='background:#888' class='btn btn-primary'>删除</a></td>";
                }else{
                    echo "<td><a href='delete.php?id={$row['id']}' class='delete btn btn-danger'>删除</a></td>";
                }
